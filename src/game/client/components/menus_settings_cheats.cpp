...
void CMenus::RenderCheats(CUIRect MainView) {
  ...
  Ui()->DoScrollbarOption(&g_Config.m_ClAimbotSensitivity,
                          &g_Config.m_ClAimbotSensitivity, &MainView,
                          Localize("Sensitivity"), 1, 360);
  MainView.HSplitTop(20.0f, nullptr, &MainView);
  ...
  if (DoButton_CheckBox(&s_EnabledToggle, Localize("Enabled"),
                        g_Config.m_ClAimbotEnabled, &Right)) {
  ...
  if (DoButton_CheckBox(&s_DrawLinesToggle, Localize("Draw Lines"),
                        g_Config.m_ClAimbotDrawLines, &Left)) {
